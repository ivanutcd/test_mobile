...
namespace utcd.cobro_prejuridico.Domain.Modules.Formulario.Feature.PublicarFormulario
{
    public class PublicarFormularioCommandHandler : ICommandHandler<PublicarFormularioCommand>
    {
        ...
        public PublicarFormularioCommandHandler(IWritableEventStoreRepository<FormularioRoot> store, IReadOnlyRepository<Projections.FormularioTable.Formulario> repositor)
        {
            Store = store;
        ...
        public async Task Handle(PublicarFormularioCommand command)
        {
            ...
            Projections.FormularioTable.Formulario formularioActual = await Repository.FirstOrDefault(actualSpec);
            ...
            while (formularioIterador.FormularioBaseId.HasValue)
            {
                ...
                Projections.FormularioTable.Formulario baseForm = await Repository.FirstOrDefault(baseSpec);
                if (baseForm == null) break;
            ...
            async Task RecorrerDescendientes(Guid parentId)
            {
                ...
                IEnumerable<Projections.FormularioTable.Formulario> formulariosHijos = await Repository.List(hijosSpec);
