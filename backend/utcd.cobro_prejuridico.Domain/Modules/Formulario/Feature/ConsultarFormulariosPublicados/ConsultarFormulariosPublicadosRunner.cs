...
public class ConsultarFormulariosPublicadosRunner
    : IQueryRunner<ConsultarFormulariosPublicadosQuery, IPaginated<FormulariosPublicadosResponse>>
{
    public IReadOnlyRepository<Projections.FormularioTable.Formulario> Repository { get; }
    public IObjectMapper Mapper { get; }

    public ConsultarFormulariosPublicadosRunner(
    ...
    public async Task<IPaginated<FormulariosPublicadosResponse>> Run(
        ConsultarFormulariosPublicadosQuery query
    )
    {
        var spec = new SpecificationGeneric<Projections.FormularioTable.Formulario>();

        if (query.Request.IdFormulario != null && query.Request.IdFormulario != Guid.Empty)
            spec.Query.Where(x => x.Id == query.Request.IdFormulario);

        if (!string.IsNullOrEmpty(query.Request.IdUsuario))
            spec.Query.Where(x => x.IdUsuario == query.Request.IdUsuario);

        spec.Query.Where(x => x.Estado == "publicado");
        ...
        IPaginated<FormulariosPublicadosResponse>? response = Mapper.Map<
            IPaginated<FormulariosPublicadosResponse>
        >(paginatedFormulario);
        foreach (FormulariosPublicadosResponse? item in response.Data)
        {
            var hashInput =
                $"{item.Id}|{item.NombreTecnico}|{item.VersionFormulario}|{item.EstructuraFormulario}|{item.UpdatedDate:o}";
